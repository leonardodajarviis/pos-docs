...
## Ví dụ Business Scenario

### Scenario 1: Vô hiệu hóa thành công mã giá độc lập

```
Hệ thống có:
- PC-100 (Nhẫn vàng 24K): Active, không có mã giá con
...
✅ Thành công
- PC-100: Inactive
- Bảng giá cũ sử dụng PC-100: Vẫn hoạt động
- Bảng giá mới: Không thể chọn PC-100
```

### Scenario 2: Không thể vô hiệu hóa vì có mã giá con Active
...
Kết quả:
❌ Lỗi: "Không thể vô hiệu hóa. Có 1 mã giá con đang Active: PC-002"

Giải pháp:
1. Deactive PC-002 trước
2. Sau đó mới Deactive PC-001
```

### Scenario 3: Vô hiệu hóa theo đúng thứ tự

```
Hệ thống có chuỗi kế thừa:
- PC-001 (gốc): Active
  └─ PC-002 (con): Active
      └─ PC-003 (cháu): Active

Quy trình đúng:
1. Deactive PC-003 (cháu) → ✅ Thành công
2. Deactive PC-002 (con) → ✅ Thành công
3. Deactive PC-001 (gốc) → ✅ Thành công

Kết quả cuối:
- PC-001, PC-002, PC-003: Tất cả đều Inactive
- Có thể Active lại theo thứ tự ngược: PC-001 → PC-002 → PC-003
```
